...
class JonesMatrix {
  public:
    ...
    JonesMatrix(double j00_r = 0.00, double j01_r = 0.00, double j10_r = 0.00, double j11_r = 0.00)
...
class Beam2016Implementation {
  public:
    struct DataSetIndex {
        DataSetIndex(char pol_, int antenna_, int freqHz_) : pol(pol_), antenna(antenna_), frequencyHz(freqHz_) {}

        bool operator<(const DataSetIndex &rhs) const {
            return std::make_tuple(pol, antenna, frequencyHz) < std::make_tuple(rhs.pol, rhs.antenna, rhs.frequencyHz);
        }

        std::string Name() const { return pol + std::to_string(antenna + 1) + '_' + std::to_string(frequencyHz); }
    ...
    //------------------------------------------------------------------------------------------------------------------
    // Calculate jones matrix in a specified direction for a given frequency, delays and amplitudes. Zenith
    // normalisation can also be disabled - but by default is enabled : This function will  :
    //    - calculate coefficients of spherical waves (SPW) if required (meaning if frequency, delays or amplitudes are
    //    different than last set of coefficients was calculated for)
    ...
    JonesMatrix CalcJones(double az_deg, double za_deg, int freq_hz_param, const double *delays, const double *amps,
                          recursive_lock<std::mutex> &lock, bool bZenithNorm = true);
    ...
    void CalcJonesArray(std::vector<std::vector<double>> &azim_arr, std::vector<std::vector<double>> &za_arr,
                        std::vector<std::vector<JonesMatrix>> &jones, int freq_hz_param, bool bZenithNorm = true);

  private:
    //---------------------------------------------------- Calculations and variables for spherical waves coefficients
    //(see equations 3-6 in the Sokolowski et al paper) ----------------------------------------------------
    ...
    // These coefficients are calculated ones for a given frequency, delays, amplitudes so these parameters are stored
    // to only calculate new coefficients when they change. X polarisation :
    struct Coefficients {
    ...
    //-------------------------------------------------------------------- Calculation of Jones matrix components for
    //given polarisation (eq. 4 and 5 in the Sokolowski et al (2017) paper --------------------------------
    // Internal function to calculate Jones matrix componenets for a given polarisation (pol). The are calculated as
    // electric field vectors E_theta_mn (eq.4) and E_phi_mn (eq.5 in the Sokolowski et al 2017 paper). INPUT :
    //    phi,theta - FEKO convention coordinates (phi=90-azim), theta=za in radians already
    //    Q1_accum, Q2_accum, M_accum, N_accum, MabsM, Nmax - coefficients calculated earlier in CalcModes function for
    //    given frequency, delays and amplitudes
    // OUTPUT :
    //    Jones matrix filled with components for given polarisation (pol input parameter)
    void CalcSigmas(double phi, double theta, const Coefficients &coefficients, char pol,
    ...
    // spherical waves coefficients (stored in the 3 variables above)
    ...
    void GetModes(int freq_hz, size_t n_ant, const double *delays, const double *amps, Coefficients &coefsX,
                  Coefficients &coefsY, recursive_lock<std::mutex> &lock);

    // function calculating all coefficients Q1, Q2, N, M and derived MabsM, Nmax for a given polarisation ("X" or "Y")
    // - perhaps enum should be used here
